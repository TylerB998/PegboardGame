...
static Pegboard pg =  new Pegboard();
 ...
 * 
 */
    //Start peg jumping, your current peg slot becomes a 0
    PegboardSolitaire.puzzleRep[startRow][startCol] = 0;
    //End spot being jumped to. It goes from a 0 to a 1 to show your new peg slot
    PegboardSolitaire.puzzleRep[endRow][endCol] = 1;
    
    int middleRow = ((startRow + endRow) / 2);
    int middleCol = ((startCol + endCol) /2);
    
    //Since middle peg is being removed, it becomes a 0
    PegboardSolitaire.puzzleRep[middleRow][middleCol] = 0;

}


/**
* Method used to check if there are any more valid moves
*/
    ...
    for(int j = 0; j < 7; j ++){
        if(PegboardSolitaire.puzzleRep[i][j] == 1){
            ...
            if( (i > 1 && j > 1 && j < 5)  || (i > 3 && j < 2 && j > 4)){
                ...
                if(PegboardSolitaire.puzzleRep[i-1][j] == 1 && PegboardSolitaire.puzzleRep[i-2][j] == 0){
                    return true;
                }
            }
            ...
            if( (j > 1 && i > 1 && i < 5)  || (j > 3 && i < 2 && i > 4)){
                //If peg exists above current
                if(PegboardSolitaire.puzzleRep[i][j-1] == 1 && PegboardSolitaire.puzzleRep[i][j-2] == 0){
                    return true;
                }
            }
...
public static boolean checkStart(int startRow,int startCol){
    ...
    if(PegboardSolitaire.puzzleRep[startRow][startCol] != 1){
...
if(PegboardSolitaire.puzzleRep[endRow][endCol] == 1){	
...
public static boolean checkEndMove(int startRow, int startCol, int endRow, int endCol){
    ...
    if(PegboardSolitaire.puzzleRep[middleRow][middleCol] != 1){
        System.out.println("Illegal move, no peg being jumped over, re-enter move.");
        return false;
    }
return true;
